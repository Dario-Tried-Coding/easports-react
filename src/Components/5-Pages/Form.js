...
function Form() {
  ...
  return (
    <>
      <Helmet>
        <title>Form - Sito ufficiale EA</title>
      </Helmet>
            ...
            <>
              ...
              <form ref={formRef} action="/">
                ...
                {/* <div className={style.checkbox}>
                  <motion.div className={style.square} animate={{ backgroundColor: checkboxChecked ? "#1f55ca" : "#141724", boxShadow: checkboxChecked ? "0 0 10px 0 rgba(255, 255, 255, 0.76)" : "0 0 0 0 rgba(255, 255, 255, 0)", transition: { backgroundColor: { duration: 0 } } }} transition={{ duration: 0.2, ease: "easeInOut" }} onClick={() => setCheckboxChecked(!checkboxChecked)}>
                    {checkboxChecked && <span>&#x2714;</span>}
                  </motion.div>
                  <label htmlFor="remember" onClick={() => setCheckboxChecked(!checkboxChecked)}>
                    Ricordami
                  </label>
                </div> */}
                <motion.button whileHover="hover" initial="rest" variants={button1Variants} type="submit">
                  ACCEDI
                </motion.button>
              </form>
              <p> Non hai un account? <button onClick={() => setOnLogin(false)}>Registrati</button></p>
            </>
          ) : (
            <>
              ...
              <form ref={formRef}>
                ...
                <div className={style.input}><input type="text" name="name" id="name" placeholder="Inserisci il tuo nome" /></div>
                <label htmlFor="surname">Cognome</label>
                <div className={style.input}><input type="text" name="surname" id="surname" placeholder="Inserisci il tuo cognome" /></div>
                <label htmlFor="email">E-mail</label>
                <div className={style.input}><input type="email" name="email" id="email" placeholder="Inserisci la tua e-mail" /></div>
                <label htmlFor="password">Password</label>
                <div className={style.input}>
                  <input type={passwordVisible ? "text" : "password"} name="password" id="password" placeholder="Inserisci la tua password" />
                  <motion.button whileHover={{scale: 0.94, backgroundColor: "#141724", transition: {backgroundColor: {duration: 0}}}} initial={{scale: 1, backgroundColor: "#343647"}} onClick={handlePasswordBtn}>
                ...
                <motion.button type="submit" whileHover="hover" initial="rest" variants={button1Variants}>Registrati</motion.button>
                <p> Sei già registrato? <button onClick={() => setOnLogin(true)}>Accedi</button></p>
              </form>
            </>
          )}
        </div>
