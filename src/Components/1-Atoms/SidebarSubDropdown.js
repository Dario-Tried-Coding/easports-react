  ...
  return (
    <motion.div className={style.subDropdown} initial={false} animate={{borderBottomColor: isOpen ? "var(--clr-dark-700)" : "var(--clr-light-900)"}}>
      ...
      <motion.ul animate={isOpen ? {height: "auto", opacity: 1} : {height: 0, opacity: 0, transition: {opacity: {duration: 0.2}}}}>
        {data.links.map((link, i) => <motion.li key={i} initial={{color: link[2] ? "var(--clr-brand-400)" : "var(--clr-dark-700)"}} whileHover={{color: "var(--clr-brand-400)"}} transition={{duration: 0}}><Link className={style.linkTag} to={link[1]}>{link[0]}</Link></motion.li>)}
      </motion.ul>
    </motion.div>
  );
}
