...
const defaultMockData = {
  heading: "Heading",
  links: [["Link 1", "/", "active"], ["Link 2", "/"], ["Link 3", "/"], ["Link 4", "/"], ["Link 5", "/"], ["Link 6", "/"]],
}

function SidebarSubDropDown({data = defaultMockData}) {
  const [isOpen, setIsOpen] = useState(data.links.some(arr => arr.includes("active")));
  ...
  return (
    <motion.div className={style.subDropdown} initial={false} animate={{borderBottomColor: isOpen ? "var(--clr-dark-700)" : "var(--clr-light-900)"}}>
      ...
      <motion.ul animate={isOpen ? {height: "auto", opacity: 1} : {height: 0, opacity: 0, transition: {opacity: {duration: 0.2}}}}>
        {data.links.map((link, i) => <motion.li key={i} initial={{color: link[2] ? "var(--clr-brand-400)" : "var(--clr-dark-700)"}} whileHover={{color: "var(--clr-brand-400)"}} transition={{duration: 0}}><Link to={link[1]}>{link[0]}</Link></motion.li>)}
      </motion.ul>
    </motion.div>
  );
}
