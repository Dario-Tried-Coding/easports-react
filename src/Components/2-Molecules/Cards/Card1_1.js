...
function Card1_1({links = defaultLinks, bgImg = "https://via.placeholder.com/767", logoImg = "https://via.placeholder.com/300x100/FFFFFF/000000", logoImgWidth = 100, gradient = false, cardLink = "/"}) {
    ...
    <>
      <BrowserView>
        <motion.div className={style.card} initial="rest" whileHover="hover" transition={{ ease: "easeOut", duration: 0.4 }}>
          <img className={style.bgImg} src={bgImg} alt="background image" />
          <div className={style.overlay} style={{backgroundColor: "black", opacity: 0.3}} ></div>
          <MotionLink className={style.overlay} to={cardLink} style={{background: gradient ? "linear-gradient(90deg, rgba(32,27,112,0.896796218487395) 0%, rgba(204,32,32,0.8995973389355743) 100%)" : "rgba(13,9,84,0.9)"}} variants={{ rest: { opacity: 0 }, hover: { opacity: 1 } }} transition={{ duration: 0.2 }}></MotionLink>
          <div className={style.content}>
            <motion.img variants={{ rest: { y: "0%" }, hover: { y: "-80%" } }} className={style.logoImg} style={{width: `${logoImgWidth}%`}} src={logoImg} alt="logo image" />
            <motion.div variants={{ rest: { opacity: 0 }, hover: { opacity: 1 } }} className={style.links}>
              {links.map((arr, i) => <MotionLink variants={{ rest: { y: `-${i * 100}%` }, hover: { y: "0%" } }} to={arr[1]} className={style.link}>{arr[0]}</MotionLink> )}
            </motion.div>
          </div>
        </motion.div>
      </BrowserView>
      <MobileView>
        <motion.div className={style.card} initial="rest" animate={isActive ? "hover" : "rest"} transition={{ ease: "easeOut", duration: 0.4 }} >
          ...
          <MotionLink className={style.overlay} to={cardLink} style={{background: gradient ? "linear-gradient(90deg, rgba(32,27,112,0.896796218487395) 0%, rgba(204,32,32,0.8995973389355743) 100%)" : "rgba(13,9,84,0.9)"}} variants={{ rest: { opacity: 0 }, hover: { opacity: 1 } }} transition={{ duration: 0.2 }}></MotionLink>
          <div className={style.content}>
            <motion.img variants={{ rest: { y: "0%" }, hover: { y: "-80%" } }} className={style.logoImg} src={logoImg} alt="logo image" />
            <motion.div variants={{ rest: { opacity: 0 }, hover: { opacity: 1 } }} className={style.links}>
              {links.map((arr, i) => <MotionLink variants={{ rest: { y: `-${i * 100}%` }, hover: { y: "0%" } }} to={arr[1]} className={style.link}>{arr[0]}</MotionLink> )}
            </motion.div>
          </div>
